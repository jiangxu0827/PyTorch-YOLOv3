# -*- coding: utf-8 -*-
"""
Created on Wed Dec  5 20:06:10 2018
...
def compute_centroids(n_anchors,loss_convergence,grid_size,iterations_num,plus):
    ...
    np_x=np.array(box_x)
    np_x=np_x.reshape(np_x.size,1)
    np_y=np.array(box_y)
    np_y=np_y.reshape(np_y.size,1)
    np_w=np.array(box_w)
    np_w=np_w.reshape(np_w.size,1)
    np_h=np.array(box_h)
    np_h=np_h.reshape(np_h.size,1)
    np_xywh=np.concatenate((np_x,np_y,np_w,np_h),axis=1)
    for box in np_xywh:
